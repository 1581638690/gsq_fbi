...
sys.path.append("lib")
import uuid
from bottle import request, Bottle, abort, route, response,redirect1,  template
import json
import time
import zipfile
import ujson
...
def callback():
    ...
    if not code:
        return ujson.dumps({"success":False,"msg":"无认证信息"})
    # if state != session.get()
    # 交换授权码以获取访问令牌
    try:
        token_response = get_access_token(clientID,clientSecret,code,state)

        # "https://iam.gongshu.gov.cn/bam-protocol-service/oauth2/getToken?code=4737d224c001ea1da2aeb0924468a9e6&cliend_id=qllsj&client_secret=5ca6d687b49f4a1cae1aeaa4aa286991&grant_type=authorization_code"
        if token_response.status_code != 200:
            response.status = token_response.status_code
            return {'error': 'Failed to obtain token',"code":response.status_code,'response': token_response.text}

        token_response_data = token_response.json()
    except Exception as e:
        return {"error":"post","msg":e.__str__()}

    access_token = token_response_data.get('access_token')

    if not access_token:
        return ujson.dumps({"success":False,"msg":"Token过期"},ensure_ascii=False)
    try:
        # 使用访问令牌获取用户信息
    ...
    # 将会话ID存放到响应头中
    #response.set_header('Session-ID', session_id)
    key= "use:APP-DLP-SE"
    # 获取到access_tokenID，获取到用户id
    ...
    if user_name and user_name in user_dic:
        user_name = user_dic.get(user_name,user_name)
        #log_login(user_data)
        #is_superuser = user_name in SUPERUSER_WHITELIST

        session = get_session_id(user_name) # 生成session会话
        ...
        #response.set_cookie("is_superuser",is_superuser)
        ...
        #return redirect1(github_auth_url)
        #return redirect1("https://59.202.68.91:8443/")
...
def get_access_token(client_id,client_secret,code,state):
    ...
    res = requests.post(url = TOKEN_URL,params=params,headers = headers)
    return res
...
def idendify_user(user_response):
    key= "use:APP-DLP-SE"
    # 获取到access_tokenID，获取到用户id
    user_data = user_response.json()
    user_name = user_data.get("user_name")
    ...
    response.set_cookie("work_space","public",path="/")
    fea_session_key = "fbi_session:%s" %(session)
    #user_info = fbi_user_mgr.get_user_by_name(user_name) # 通过姓名获取

    ssdb0.set(fea_session_key,"%s:%s"%(user_name,""))
    #build_sysrule(ssdb0,user_info)
    return redirect1("https://59.202.68.91:8443/wap.h5?key=%s"%key)
    #else:
        #return '{"message":"token incorrect,access denied"}'
#curl -v -X POST 'https://iam.gongshu.gov.cn/bam-protocol-service/oauth2/getToken?client_secret=5ca6d687b49f4a1cae1aeaa4aa286991&code=7354f67d32263ffd8254e635687f5b29&grant_type=authorization_code&client_id=qllsj'
