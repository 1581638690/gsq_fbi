...
fh =  RotatingFileHandler('logs/esql.log', maxBytes=10*1024*1024,backupCount=3)
...
class ESql_query():
	...
	def do_scan(self):
		self.scan_options={"size":1000,"routing":None}
	...
	def matchall_search(self):	
		...
		#res = self.es.search(request_timeout=100,index=self.tables, doc_type=self.doc_types, body={"query": {"match_all": {}}},from_=self.begin,size=self.size, _source_include=self.fields,ignore_unavailable=True)
	
		return res
	
	def matchall_scan(self):
		if  self.scan_options["routing"] ==None:	
			res = self.es.search(request_timeout=100,index=self.tables, doc_type=self.doc_types , body={"query": {"match_all": {}}},size=self.scan_options["size"], search_type="scan",scroll="1m", _source_include=self.fields)
			#res = self.es.search(request_timeout=100,index=self.tables,  body={"query": {"match_all": {}}},size=self.scan_options["size"],scroll="10m", _source_include=self.fields)
		else:
			res = self.es.search(request_timeout=100,index=self.tables, doc_type=self.doc_types ,body={"query": {"match_all": {}}},size=self.scan_options["size"], preference="_shards:%s"%(self.scan_options["routing"]),search_type="scan", scroll="1m", _source_include=self.fields)
	...
	def query_scan(self):
		if  self.scan_options["routing"] ==None:
			res = self.es.search(request_timeout=100, index=self.tables, doc_type=self.doc_types,body=self.build_body(),size=self.scan_options["size"], search_type="scan",scroll="1m", _source_include=self.fields)
		else:
			res = self.es.search(request_timeout=100, index=self.tables,doc_type=self.doc_types ,body=self.build_body(),size=self.scan_options["size"], preference="_shards:%s"%(self.scan_options["routing"]), search_type="scan",scroll="1m", _source_include=self.fields)
		return res
	...
	def parse_sql(self,sql):
		...
		if len(sql_tree["scan"])!=0:
			str_scan = "".join(self.sql_tree["scan"])
			self.fields = str_scan.split(",")
