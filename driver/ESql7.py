	...
	for cond in sql_tree["where"]["must"]:
		...
		if cond.find(".length()") > 0:
			...
			body["query"]["bool"]["must"].append(
				{"regexp": {key_value[0].strip(): {"value": ".{%s,}" % (key_value[1].strip())}}})
			continue
				...
				if len(a) >= 2:
					...
					if len(fds) >= 2:
						...
						body["query"]["bool"]["must_not"].append(
							{"query_string": {"fields": fields, "query": a[1].strip()}})
					else:
						body["query"]["bool"]["must_not"].append(
							{"query_string": {"default_field": a[0].strip(), "query": a[1].strip()}})
				else:
					a = cond.split("=")
					if len(a) >= 2:
						...
						if a[0].strip() == "q":
							...
							if len(fds) >= 2:
								...
								body["query"]["bool"]["must"].append({"query_string": {"default_field": a[0].strip(),
																					   "query": value,
					...
					if len(a) >= 2:
						...
						if a[0].strip() == "q":
							...
							if len(fds) >= 2:
								...
								for fd in fds:
									fields.append(fd.strip())
								body["query"]["bool"]["should"].append(
									{"fields": {"default_field": fields, "query": value}})
							else:
								body["query"]["bool"]["should"].append(
									{"query_string": {"default_field": a[0].strip(), "query": value}})
						...
						if b > 0:
							...
							for mark in [" gte ", " gt ", " lte ", " lt "]:
								...
								if len(a) >= 2:
									body["query"]["bool"]["should"].append(
										{"range": {a[0].strip(): {mark.strip(): a[1].strip()}}})
									break
...
class ESql_ddl():
	...
	def do_sql(self, sql):
		...
		try:
			if sql.find("show tables") == 0:
				...
				res = self.do_show_fun(self.es.cat.nodes,
	...
	def do_show_meminfo(self, sql):
		...
		try:
			...
			field_mems = fielddata.loc[:, ["_parent", "ip", "field"]]

			df = seg_mems.merge(field_mems, left_index=True, right_on="ip")

			df = df.rename(columns={"_parent": "parent", "field": "fielddata",
	...
	def do_readonly(self, sql):
		...
		result = {"type": "ddl", "count": 0, "took": 20, "result": [], "msg": ""}
		try:
			res = self.es.indices.put_settings(index=[i.strip() for i in l],
	...
	def do_alter_meta(self, sql):
		try:
			...
			if doc_type == "_self":  # sample: alter meta vehicle_type._self(车大表)
				...
				res = self.es.index(index="sys_meta", doc_type="base", body=doc,
									id="%s:%s:%s" % (index, doc_type, index))
			else:
				# add by gjw
				if doc_type != "_type":
					...
					for field in fields:
						k_v = field.split("=")
						b = {"index": {"_index": "sys_meta", "_type": "base",
									   "_id": "%s:%s:%s" % (index, doc_type, k_v[0].strip())}}
						doc = {
							'index': index,
							'type': doc_type,
							"field": k_v[0].strip(),
	...
	def do_get_meta(self, sql):
		...
		try:
			if index_name == "*":  # add by gjw on 20140916
				...
				sql = 'select field,zh,cc from sys_meta where index="%s" and type="%s" limit 5120' % (
					index_name, doc_type)
	...
	def do_showtables(self, sql):
		...
		# add a sum to result
		d = {"_index": "total", "docs.deleted": 0, "docs.count": 0, "store.size": 0, "pri.store.size": 0, "pri": 0,
			 "rep": 0}
		for r in result["result"]:
			try:
...
class ESql_query():
	...
	def do_scan(self):
		...
		res = helpers.scan(self.es, query=body, index=self.tables, doc_type="_doc", _source=self.fields,
	...
	def matchall_search(self):
		...
		res = self.es.search(request_timeout=100, index=self.tables, body={"query": {"match_all": {}}}, \
							 from_=self.begin, size=self.size, _source=self.fields, sort=self.sql_tree["order"],
							 ignore_unavailable=True)
		res2 = self.es.count(request_timeout=100, index=self.tables, body={"query": {"match_all": {}}},
							 ignore_unavailable=True)
	...
	# add by 丁国益 2020/12/24
	def build_query_body2(self, flag, where_cond):
		body = {
			"bool": {
				"must": [],
				"must_not": [],
			...
			if isinstance(cond, dict):
				...
				for cond_key, cond_value in cond.items():
					...
					if len(cond_value) > 0:
						...
						tt = self.build_query_body2(cond_key, cond_value)
						body["bool"][flag].append(tt)
			else:
				if cond.find(".length()") > 0:
					cond = cond.replace(".length()", "")
					key_value = cond.split(">=")
					body["bool"][flag].append(
						{"regexp": {key_value[0].strip(): {"value": ".{%s,}" % (key_value[1].strip())}}})
					continue
				cond = cond.replace(">=", " gte ").replace(">", " gt ").replace("<=", " lte ").replace("<", " lt ")
				n = cond.find(" not in ")
				if n > 0:
					fname, tags = deal_in_not_in(cond, n, len(" not in "))
					body["bool"]["must_not"].append({"terms": {fname: tags}})
				else:
					n = cond.find(" in ")
					n1 = cond.find(" ins ")
					if n > 0:
						fname, tags = deal_in_not_in(cond, n, len(" in "))
						body["bool"][flag].append({"terms": {fname: tags}})
					elif n1 > 0:
						fname, tags = deal_in_not_in(cond, n, len(" ins "))
						body["bool"][flag].append(
							{"terms": {fname: tags, "minimum_should_match": len(tags)}})
					else:
						a = cond.split("!=")
						if len(a) >= 2:
							fds = a[0].split(",")
							...
							if len(a) >= 2:
								...
								if a[0].strip() == "q":
									score_weight = 100
									fields = []
									if ("score_by" in self.sql_tree) == False:
										self.sql_tree["score_by"] = []
									for score_field in self.sql_tree["score_by"]:
										fields.append("%s^%d" % (score_field, score_weight))
										score_weight = score_weight / 2
									fields.append("_all")
									body["bool"][flag].append(
									...
									if len(fds) >= 2:
										...
										if flag == 'must':
											body["bool"]["must"].append(
												{"query_string": {"fields": fields, "query": value,
																  "default_operator": "AND"}})
										elif flag == 'should':
										...
										if flag == 'must':
											body["bool"]["must"].append(
												{"query_string": {"default_field": a[0].strip(),
																  "query": value,
																  "default_operator": "AND"}})
										elif flag == 'should':
											body["bool"]["should"].append(
								...
								if b > 0:
									...
									b2 = cond[t + len(" to "):].strip()
									body["bool"][flag].append(
										{"range": {field_name: {"gte": b1, "lte": b2}}})
								else:
									if flag == 'must':
										n = cond.find("is null")
										if n > 0:
		...
		if flag == 'must':
			for k, v in list(range_conds.items()):
				if len(v) == 1:
					body["bool"]["must"].append({"range": {k: {v[0][0]: v[0][1]}}})
				else:
					body["bool"]["must"].append({"range": {k: {v[0][0]: v[0][1], v[1][0]: v[1][1]}}})
	...
	# add by 丁国益 2020/12/24
	def build_query_body3(self, body):
		if "should" in body["query"]["bool"] and len(body["query"]["bool"]["should"]) >= 2:
			body["query"]["bool"]["minimum_should_match"] = 1

		if "highlight" in self.sql_tree and len(self.sql_tree["highlight"]) > 0:
			fields = "".join(self.sql_tree["highlight"])
			h = {"fields": {}}
			for field in fields.split(","):
				if field == "": continue
				# add by qs on 2016-01-15 20.8.68
	...
	def build_body(self):
		...
		if (len(self.doc_types) >= 1 and self.doc_types[0] == "ALL"):
			...
			Mainbody["query"] = Childbody
			xx = build_parent_child_query_body(Mainbody, self.sql_tree)
			return xx
		if self.sql_tree['where'] == None:
			Mainbody["query"] = Childbody
			build_query_body(Mainbody, self.sql_tree)
		else:
			# print(self.sql_tree['where'])
			for key, value in self.sql_tree['where'].items():
				# print('key', key)
				# print('value', value)
				tt1 = self.build_query_body2(key, value)

			# Childbody["bool"][key].append(tt)
			Mainbody["query"] = tt1
			Mainbody = self.build_query_body3(Mainbody)
		#delete by gjw on 2021-0422
		#Mainbody = json.dumps(Mainbody, ensure_ascii=False)
		# print('Mainbody', Mainbody)
		# print('sql_tree', self.sql_tree)
		return Mainbody

	def query_search(self, body):
		# update by qs 20200730
		res = self.es.search(request_timeout=100, index=self.tables, doc_type=self.doc_types, body=body,
							 from_=self.begin, size=self.size, _source=self.fields, sort=self.sql_tree["order"],
							 ignore_unavailable=True)
	...
	def query_search_order(self, body):
		res = self.es.search(request_timeout=100, index=self.tables, doc_type=self.doc_types,
							 body=body, from_=self.begin, size=self.size,
	...
	def query_scan(self):
		if self.scan_options["routing"] == None:
			res = self.es.search(request_timeout=100, index=self.tables, doc_type=self.doc_types,
								 body=self.build_body(), size=self.scan_options["size"], search_type="scan",
								 scroll="1m", _source=self.fields, ignore_unavailable=True)
		else:
			res = self.es.search(request_timeout=100, index=self.tables, doc_type=self.doc_types,
								 body=self.build_body(), size=self.scan_options["size"],
								 preference="_shards:%s" % (self.scan_options["routing"]), search_type="scan",
								 scroll="1m", _source=self.fields, ignore_unavailable=True)
		return res
	...
	def parse_sql(self, sql):
		words = sql.split("where")
		words_bf_where = words[0].split(" ")
		if len(words) > 1:
			R1 = re.compile(' +(?![^(]*\))')
			words_bh_where = R1.split(words[1])
			words_bf_where.append('where')
			words = words_bf_where + words_bh_where
		else:
			words = words_bf_where
		sql_tree = {}
		...
		# 处理where
		# add by gjw 20140721
		# update by 丁国益 2020/12/24  内容：下方if-else 代码块
		if len(sql_tree["where"]) == 0:
			sql_tree["where"] = None  # 没有where
		else:
			# print(sql_tree['where'])
			where_cond = " ".join(sql_tree["where"])
			# print('where_cond', where_cond)
			cond = self.parse_where(where_cond)
			sql_tree["where"] = cond
		...
		# print('sql_tree', sql_tree)
		return sql_tree

	# update by 丁国益 2020/1/5
	def parse_where(self, where_cond):
		# print('ss')
		# R2 = re.compile("(?<!\([^)]*)( +or +)(?![^(]*\))")
		# R2 = re.compile('( or )')
		R4 = re.compile('\((.*?)\)')
		R5 = re.compile('( and )|( or )')
		left_par_site = []
		left_par_num = 0
		right_par_site = []
		right_par_num = 0
		# 括号以及里面的内容先替换掉，这样，每次都是一层sql
		for char in range(len(where_cond)):
			if where_cond[char] == '(':
				if left_par_num == 0:
					left_par_site.append(char)
				left_par_num += 1
			if where_cond[char] == ')':
				right_par_num += 1
				if right_par_num == left_par_num:
					right_par_site.append(char)
					left_par_num = 0
					right_par_num = 0

		split_dict = {}
		for i in range(len(left_par_site)):
			# print(sql_word2[left_par_site[i]:right_par_site[i] + 1])
			key = "split_dict" + str(i)
			split_dict[key] = where_cond[left_par_site[i]:right_par_site[i] + 1]

		# 如果有括号
		if len(left_par_site) > 0:
			for key, value in split_dict.items():
				where_cond = where_cond.replace(value, key)
		# print('where_cond',where_cond)
		# print('split_dict', split_dict)
		# 先解析or
		cond_list = where_cond.split(' or ')
		cond_list = [i.strip() for i in cond_list]
		# 如果列表长度大于1，说明能被or分割。否则说明第一层sql语句的逻辑连接符就只有and,或者只有一个条件
		if len(cond_list) == 1:
			flag = 'must'
			child_and_list = []
			# 没有or,那么第一个元素则是他全部的sql语句
			and_list = cond_list[0].split(' and ')
			and_list = [i.strip() for i in and_list]
			# print('must   and_list', and_list)
			for num in range(len(and_list)):
				if 'split_dict' not in and_list[num]:
					child_and_list.append(and_list[num])
				else:
					# 将括号内容替换回来
					if ' in ' in and_list[num]:
						in_list = and_list[num].split(' in ')
						for num in range(len(in_list)):
							if 'split_dict' in in_list[num]:
								in_list[num] = split_dict[in_list[num]]
						cond_value = ' in '.join(in_list)
						child_and_list.append(cond_value)
					else:
						cond_value = split_dict[and_list[num]]
						# 去除两边的空格以及括号
						cond_value = cond_value.strip()[1:-1].strip()
						# print('next_char', cond_value)
						x2 = self.parse_where(cond_value)
						# print('x2', x2)
						child_and_list.append(x2)
			# print('must:child_and_list',child_and_list)
			cond_list = child_and_list
			...
			for index in range(len(cond_list)):
				# 由于被or分割，所以剩下的有3种情况，1.and连接的条件。2.单独的or条件。3.替代括号的字符串。
				and_list = cond_list[index].split(' and ')
				# 情况1
				if len(and_list) >= 2:
					...
					for num in range(len(and_list)):
						if 'split_dict' not in and_list[num]:
							...
							if ' in ' in and_list[num]:
								...
								# print('x2', x2)
								child_and_list.append(x2)
					# print('should:child_and_list', child_and_list)
					cond_list[index] = {}
					cond_list[index]["must"] = child_and_list
				# 情况3，若是情况2，不用改变
				elif len(and_list) == 1 and 'split_dict' in and_list[0]:
					# 将括号内容替换回来
					cond_value = split_dict[and_list[0]]
					# 去除两边的空格以及括号
					cond_value = cond_value.strip()[1:-1].strip()
					# print('should:cond_value',cond_value)
					# 将其递归
					x2 = self.parse_where(cond_value)
					# print('should:x2', x2)
					cond_list[index] = x2
...
class ESql_aggs():
	...
	def do_sql(self, sql):
		...
		logger.info("return result  count=%s, took=%sms, cost=%sms" % (
			result["count"], result["took"], result["cost"]))  # add on 20150210
		return result
	...
	def do_aggs(self):
		...
		if body["aggs"] == {}:
			...
			res = self.es.search(request_timeout=100, index=self.tables, body=body, size=self.size,
								 ignore_unavailable=True, sort=["doc_count: desc"])
	...
	def build_qbody(self, body):
		...
		for key, value in self.sql_tree['where'].items():
			...
			# Childbody["bool"][key].append(tt)
			body["query"] = tt1
		body = self.build_query_body3(body)
		#modify by gjw on 20210425 不用这种方式
