...
class ESql():
	...
	def do_check_before(self,sql):	#add by gjw on 20150403
		for redkey in self.redkeys:
			if sql.find(redkey) >=0:
				return True
		return False
	
	def do_check_after(self,result):	#add by gjw on 20150403		
		a={}
		i=0
	...
	def do_sql(self,sql):		
		...
		"""
		if len(self.redkeys) >0:	#have the redkey to check
			if self.do_check_before(sql):
				return result
		"""
		
		#modify by gjw on 20150505 add the sql.find("select") ==0 and
		...
		"""
		if len(self.redkeys) >0:
			result = self.do_check_after(result)
		"""
...
class ESql_ddl():
	...
	def do_sql(self,sql):
		...
		try:
			...
			elif sql.find("service")==0:
				res= self.do_service(sql)
			...
			elif sql.find("reload")==0: #add by gjw on 20150403
				res = self.do_reload()
			else:
	...
	def do_sql2(self,sql):
		...
		if sql.find(" in (select") >0 or sql.find(" in ( select") >0:
			return self.ddl.do_in_child_query(sql)
		elif sql.find("count(*)") >0 or sql.find("group by") >0 or sql.find(" as ") >0:
			return self.aggs.do_sql(sql)
		elif sql.find("union all") >0:
			return self.ddl.do_sql(sql)
		elif sql.find("select") ==0 or sql.find("scan")==0 or sql.find("scroll")==0:
			return self.query.do_sql(sql)
		else:
			return self.ddl.do_sql(sql)
	...
	def do_createtable(self,sql):
		...
		try:
			...
			name string no {null_value=none,analyzer=pinyin}, sstart integer yes
			)
			"""
			sql_body = words[0].replace(")","") 
			
			sql_body = trans_ct_sql(sql_body)
			
			print(sql_body)
			items = sql_body.split(",")
			...
			for item in items:
				...
				if (len(field_sub)==2):# have the sub like field_name.sub
					if field_sub[0] not in body[type_name]["properties"]:
						body[type_name]["properties"][field_sub[0]]={"type":field[1].strip()}
					body[type_name]["properties"][field_sub[0]]["fields"]={
					field_sub[1]:{"type":field[1].strip(),"index": "not_analyzed" if field[2].strip()=="yes" else "analyzed"}
					}
					for s in field[3:]:
						#add by gjw on 20150309
						s = s.replace("`"," ")
						if s=="sortable":
							body[type_name]["properties"][field_sub[0]]["fields"][field_sub[1]]["doc_values"]=True	
						else:
							k,v = s.split("=")
							body[type_name]["properties"][field_sub[0]]["fields"][field_sub[1]][k]=v
				else:
					body[type_name]["properties"][field_sub[0]]={"type":field[1].strip()}
					...
					for s in field[3:]:
						...
						s = s.replace("`"," ")
						if s=="sortable":
							body[type_name]["properties"][field_sub[0]]["doc_values"]=True
						else:
							k,v = s.split("=")
							body[type_name]["properties"][field_sub[0]][k]=v
	...
	def do_optimize(self,sql):	
		...
		result={"type":"ddl","count":0,"took":20,"result":[],"msg":""}
		try:
			res = self.es.indices.optimize(index=[ i.strip() for i in l],max_num_segments=2,wait_for_merge=False)			
	...
	#add by gjw on 20150316
	#do service start|stop|status with [nodeip1,nodeip2]
	def do_service(self,sql):	
		l=sql[len("service "):].split("with")
		action = l[0].strip()
		if len(l) >=2:
			nodes_1= l[1].split(",")			
			nodes=[]
			for node in nodes_1:
				nodes.append({"host":node.strip()})
		else:
			nodes = cfg.udbserver
		result={"type":"ddl","count":0,"took":20,"result":[],"msg":""}
		try:
			for server in nodes:
				try:
					r = http.request('GET', 'http://%s:8001/service/%s'%(server["host"],action))
					if r.status ==200:
						d = json.loads(r.data)
						d["ip"]=server["host"]
					else:
						d={"ip":server["host"],"error":"can't get the info!"}
	...
	def do_desctable(self,sql):
		...
		try:
			...
			#print res
			#modify by gjw on 20150402
			if isinstance(res,dict):				
				a = res[list(res.keys())[0]]["mappings"]
			
			result["result"]=[]			
			for k,v in list(a.items()): # atype:properties{}
				for k1,v1 in list(v["properties"].items()):
					d = {"format":"","index":"","store":"","doc_values":""}
					d["_type"]=k
					d["field"]= k1
					for k2,v2 in list(v1.items()):
						#add by gjw on 20150324 deal the mulitfieds
						if k2 == "fields":							
							for k3, v3 in list(v2.items()):
								d2={"format":"","index":"","store":"","doc_values":""}
								d2["field"] = "%s.%s"%(k1,k3)
								d2["_type"]=k
								for k4,v4 in list(v3.items()):
									d2[k4]=v4
								result["result"].append(d2)
						else:	
							d[k2]=v2
					if "type" not in d:
						d["type"]="Object"				
	...
	def do_insert_into(self,sql):
		try:
			...
			l=sql.find("(")
			index_type = sql[b:l].strip().split(".")
	...
	def do_showtables(self,sql):
		...
		#add a sum to result
		d = {"index":"total","docs.deleted":0,"docs.count":0,"store.size":0,"pri.store.size":0,"pri":0,"rep":0}
		
		#add by gjw on v1.9.10 20140916
		res = self.do_sql("get meta *")
		meta ={}
		for r in res["result"]:
			meta[r["field"]]=[r["zh"]]
	...
	def do_show_cpuinfo(self,sql):
		...
		for server in cfg.udbserver:
			try:
				r = http.request('GET', 'http://%s:8001/cpuinfo'%(server["host"]))
				if r.status ==200:
					d = json.loads(r.data)
					d["ip"]=server["host"]
				else:
...
class ESql_query():
	...
	def do_select(self):
		...
		#add by gjw on 20150209
		if len(self.sql_tree["order"]) >0 and result["count"] >0 :			
			if res['hits']['total'] <10000: # memory order
				b1 = self.begin
				s1 = self.size
				self.begin = 0
				self.size = res['hits']['total']
				res2 = self.query_search(body)
				
				m = self.transfor_hits(res2['hits']['hits'])
				dr1 = pd.DataFrame(m).sort(columns=self.sql_tree["order2_fields"])			
				
				#add by gjw on 20150210
				if self.sql_tree["order2_flag"]=="asc":
					e = self.size if b1+s1 > self.size else b1+s1
					num_range= list(range(b1,e))
				else:
					b = 0 if self.size-b1-s1 <0 else  self.size-b1-s1
					num_range=list(range(b,self.size-b1))[::-1]
				#print num_range
				#trans the 矩阵 to json		
				m2=[]
				for r in num_range:
					a = dr1.values[r]
					d={}
					...
					m2.append(d)
				result["result"]= m2
				result["took"] = res["took"]+res2['took']
			else: #ES order
				res2 =  self.query_search_order(body)
				result["took"] = res["took"]+res2['took']
				result["result"] = self.transfor_hits(res2['hits']['hits'])
		...
		for b in hits:
			...
			if "fields" in b:
				for (k,v) in list(b["fields"].items()):
					...
					d[k] = v[0]
			"""
			if b.has_key("highlight"):
				for (k,v) in b["highlight"].items():
					d["HL_"+k]=v[0]
	...
	def transfor_hits_compact(self,hits):
		ret={}
		ret["data"]=[]
		ret["index"]=[]
		
		ret["cols"]=["_type"]+list(hits[0]["_source"].keys())
		for b in hits:
			ret["index"].append(b["_id"])
			a=[b["_type"]]
			a += list(b["_source"].values())
			"""
			if b.has_key("fields"):
				a +=b["fields"]
			"""
	...
	def do_scroll(self):
		"""
		 sql sample :
		 "scroll from c2NhbjsxOzE0Mjg1NDpOenc2TTJMMVN5cVpVcXREanppV0F3OzE7dG90YWxfaGl0czoyNjI7"
		"""
		...
		if result["count"] >0:
			result["result"] = self.transfor_hits(res['hits']['hits'])	
		#result["result"] = res['hits']['hits']	
		return result	
	
	def matchall_search(self):		
		#modify by gjw on 20150209
		...
		res = self.es.search(request_timeout=100,index=self.tables, 
			doc_type=self.doc_types, body={"query": {"match_all": {}}},
			from_=self.begin,size=self.size, _source_include=self.fields,
	...
	def matchall_scan(self):
		if  self.scan_options["routing"] ==None:	
			res = self.es.search(request_timeout=100,index=self.tables, doc_type=self.doc_types, body={"query": {"match_all": {}}},size=self.scan_options["size"], search_type="scan",scroll="10m", _source_include=self.fields)
		else:
			res = self.es.search(request_timeout=100,index=self.tables, doc_type=self.doc_types, body={"query": {"match_all": {}}},size=self.scan_options["size"], preference="_shards:%s"%(self.scan_options["routing"]), search_type="scan",scroll="10m", _source_include=self.fields)
	...
	def query_search(self,body):
		...
		res = self.es.search(request_timeout=100, index=self.tables,doc_type=self.doc_types ,body=body,from_=self.begin,size=self.size, _source_include=self.fields,ignore_unavailable=True)
	...
	def query_search_order(self,body):
		res = self.es.search(request_timeout=100, index=self.tables,
			...
			_source_include=self.fields, sort=self.sql_tree["order"],
			ignore_unavailable=True)
	...
	def query_scan(self):
		if  self.scan_options["routing"] ==None:
			res = self.es.search(request_timeout=100, doc_type=self.doc_types, index=self.tables,body=self.build_body(),size=self.scan_options["size"], search_type="scan",scroll="10m", _source_include=self.fields)
		else:
			res = self.es.search(request_timeout=100, doc_type=self.doc_types, index=self.tables,body=self.build_body(),size=self.scan_options["size"], preference="_shards:%s"%(self.scan_options["routing"]), search_type="scan",scroll="10m", _source_include=self.fields)
		return res
	...
	def parse_sql(self,sql):
		...
		if "select" in self.sql_tree:
			str_select = "".join(self.sql_tree["select"])
			self.fields = str_select.split(",")
		#add by gjw on 20151224
		if "scan" in self.sql_tree:
			str_select = "".join(self.sql_tree["scan"])
			self.fields = str_select.split(",")
...
class ESql_aggs():
	...
	def do_aggs(self):
		...
		#print  "query time",int((time.time()-self.b0)*1000)
		#print res
		#print "exec over!"
		...
		if "aggregations" in res:
			...
			if res["hits"]["total"] >0: # modify by gjw on 20150210
				result["result"] =[{"count":res["hits"]["total"]}]
		#end if 				
		#print "result data over!"
		
		#print  "trans result time",int((time.time()-self.b0)*1000)
...
def scan_data(routing=0):
	...
	#scan_sql = "scan * from inx_2015-08 with size=5000 and routing = %s" %(routing)
	scan_sql = "scan * from overlookcdesqllog where gro=info and type_name=select and day >=2016-04-01 and day <=2016-04-07 with size=500 )"
	print(scan_sql)
	res = esql.do_sql(scan_sql)
...
def startT():
	...
	ts=[]
	for i in range(5):
		t = threading.Thread(target=scan_data,args=(i,))
		t.start()
		ts.append(t)
