...
# -*- coding: utf-8 -*- 
#xlink的py文件
#filename: test_unix_recv.xlk
#datetime: 2024-08-30T16:10:58.484148
#copyright: OpenFBI
...
def flow(k,df):
	...
	try:
		...
		errors.append('[test_unix_recv.xlk]执行第[74]原语 df2 = group df by dest_ip... 出错,原因:'+e.__str__())

	ptree={'runtime': runtime,'work_space': workspace, '': '= @udf', 'Ta': 'df3', 'Action': '@udf', '@udf': 'df', 'by': 'udf0.df_sum'}
	try:
		ret = udf_func(ptree,p=2)
	except Exception as e:
		errors.append('[test_unix_recv.xlk]执行第[75]原语 df3 = @udf df by udf0.df_... 出错,原因:'+e.__str__())
	...
	try:
		...
		errors.append('[test_unix_recv.xlk]执行第[76]原语 store df2 to ssdb by ssdb... 出错,原因:'+e.__str__())
	...
	try:
		...
		errors.append('[test_unix_recv.xlk]执行第[77]原语 store df3 to ssdb by ssdb... 出错,原因:'+e.__str__())

	ptree={'runtime': runtime,'work_space': workspace, 'Action': '@udf', '@udf': 'flow', 'by': 'CRUD.save_table', 'with': 'msyql,df'}
	try:
		ret = udf_func(ptree,p=2)
	except Exception as e:
		errors.append('[test_unix_recv.xlk]执行第[78]原语 @udf flow by CRUD.save_ta... 出错,原因:'+e.__str__())
	...
	try:
		...
		errors.append('[test_unix_recv.xlk]执行第[79]原语 assert df by df.index.siz... 出错,原因:'+e.__str__())

	ptree={'runtime': runtime,'work_space': workspace, 'Action': 'drop', 'drop': 'df'}
	try:
		drop_table(ptree)
	except Exception as e:
		errors.append('[test_unix_recv.xlk]执行第[80]原语 drop df... 出错,原因:'+e.__str__())
...
def http(k,df):
	...
	try:
		...
		errors.append('[test_unix_recv.xlk]执行第[84]原语 df2 = group df by dest_ip... 出错,原因:'+e.__str__())

	ptree={'runtime': runtime,'work_space': workspace, '': '= @udf', 'Ta': 'df3', 'Action': '@udf', '@udf': 'df', 'by': 'udf0.df_sum'}
	try:
		ret = udf_func(ptree,p=2)
	except Exception as e:
		errors.append('[test_unix_recv.xlk]执行第[85]原语 df3 = @udf df by udf0.df_... 出错,原因:'+e.__str__())
	...
	try:
		...
		errors.append('[test_unix_recv.xlk]执行第[86]原语 store df2 to ssdb by ssdb... 出错,原因:'+e.__str__())
	...
	try:
		...
		errors.append('[test_unix_recv.xlk]执行第[87]原语 store df3 to ssdb by ssdb... 出错,原因:'+e.__str__())
	...
	try:
		...
		errors.append('[test_unix_recv.xlk]执行第[88]原语 t2 = @sdf format_timestam... 出错,原因:'+e.__str__())
	...
	try:
		...
		errors.append('[test_unix_recv.xlk]执行第[90]原语 assert True as xlink to 调... 出错,原因:'+e.__str__())

	ptree={'runtime': runtime,'work_space': workspace, 'Action': 'drop', 'drop': 'df'}
	try:
		drop_table(ptree)
	except Exception as e:
		errors.append('[test_unix_recv.xlk]执行第[91]原语 drop df... 出错,原因:'+e.__str__())
...
def save(k='1',df=pd.DataFrame()):
	...
	try:
		...
		errors.append('[test_unix_recv.xlk]执行第[98]原语 drop flow... 出错,原因:'+e.__str__())

	ptree={'runtime': runtime,'work_space': workspace, 'Action': 'drop', 'drop': 'df'}
	try:
		drop_table(ptree)
	except Exception as e:
		errors.append('[test_unix_recv.xlk]执行第[99]原语 drop df... 出错,原因:'+e.__str__())
