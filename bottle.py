...
__version__ = '0.12.9_gjw'
__license__ = 'MIT'

#modify by gjw on 20160107 
"""
add response get_cookies and copy_cookies
modify the static_file copy the response.cookies


1 modify by gjw on 20160329
可以设置response对象的返回码
response.set_status(301)

2 modify by gjw on 20170220
支持python3和中文编码
"""
...
if py3k:
    ...
    urlunquote = functools.partial(urlunquote, encoding='utf8')
...
class Router(object):
    ...
    def __init__(self, strict=False):
        ...
        self.filters = {
            ...
            'raw':  lambda conf: (r'.+?', None, lambda x: str(x))}
    ...
    def _handle(self, environ):
        ...
        if py3k:
            try:
                ...
                environ['PATH_INFO'] = path.encode('utf8').decode('utf8')
            except UnicodeError:
                # add by gjw on 20170215 处理中文文件下载
                import base64
                path2 = path.split("/")
                environ['PATH_INFO'] = "/%s/%s" %(base64.b64encode(path2[1].encode('utf8')).decode('latin1'),
                       base64.b64encode(path2[2].encode('utf8')).decode('latin1'))
...
class BaseRequest(object):
    ...
    def query(self):
        ...
        #modify by gjw on 20170308
        #pairs = _parse_qsl(tonat(self.environ.get('QUERY_STRING', ''),'latin1'))
    ...
    def POST(self):
        ...
        if not self.content_type.startswith('multipart/'):
            ...
            #pairs = _parse_qsl(tonat(self._get_body_string(), 'utf8'))
            for key, value in pairs:
                post[key] = value
            return post
...
class BaseResponse(object):
    ...
    #add by gjw on 20160329 becuase the routeing 
    ...
    def headerlist(self):
        ...
        if self._cookies:
            for c in self._cookies.values():
                #支持https
                out.append(('Set-Cookie', c.OutputString()+"; SameSite=None; Secure"))
        return out
    ...
    def set_cookie(self, name, value, secret=None, **options):
        ...
        #add by gjw on 20220622 cookie只读的设置
        if  name=="fbi_session":       
    ...
    #add by gjw on 20160107
    def get_cookies(self):
        return self._cookies;

    def copy_cookies(self,cookies):
...
def redirect(url, code=None):
    ...
    res.status = code
    res.body = ""
    #add by gjw on 20201105,用户https的跳转
    #http_url = urljoin(request.url, url)   
    #https_url = http_url.replace("http://","https://")
    #https_url = https_url.replace("9999","")
    #add by gjw on 202100913 
    host = request.get_header("HOST")
    https_url = "https://%s%s"%(host,url)
    res.set_header('Location', https_url)
    raise res


def redirect1(url, code=None):
    """ Aborts execution and causes a 303 or 302 redirect, depending on
        the HTTP protocol version. """
    if not code:
        code = 303 if request.get('SERVER_PROTOCOL') == "HTTP/1.1" else 302
    ...
    res.body = ""
    http_url = urljoin(request.url, url)   
    res.set_header('Location', http_url)
    raise res
...
def static_file(filename, root, mimetype='auto', download=False, charset='UTF-8'):
    ...
    if ims is not None and ims >= int(stats.st_mtime):
        ...
        #add by gjw on 20160107 copy the cookies
        static_response = HTTPResponse(status=304, **headers)
        if response.get_cookies():
            static_response.set_header("Cache-Control","no-cache")
            static_response.copy_cookies(response.get_cookies())
    ...
    #add by gjw on 20160107 copy the cookies
    static_response = HTTPResponse(body, **headers)
    if response.get_cookies():
        static_response.set_header("Cache-Control","no-cache")
        static_response.copy_cookies(response.get_cookies())
    return static_response

def download_file(filename, root, mimetype='auto', download_name="", charset='UTF-8'):
    """ Open a file in a safe way and return :exc:`HTTPResponse` with status
        code 200, 305, 403 or 404. The ``Content-Type``, ``Content-Encoding``,
        ``Content-Length`` and ``Last-Modified`` headers are set if possible.
        Special support for ``If-Modified-Since``, ``Range`` and ``HEAD``
...
class PasteServer(ServerAdapter):
    def run(self, handler): # pragma: no cover
        ...
        handler = TransLogger(handler, setup_console_handler=(self.quiet))
